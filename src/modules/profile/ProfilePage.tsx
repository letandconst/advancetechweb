...
import { AvatarUploadField, PasswordInput, ProfileReadonlyField, ProfileSectionCard } from './components'
    ...
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
        <aside className="xl:sticky xl:top-6 xl:self-start">
          <section className="overflow-hidden rounded-[32px] border border-slate-200/80 bg-[radial-gradient(circle_at_top_left,_rgba(14,165,233,0.18),_transparent_42%),linear-gradient(135deg,_rgba(255,255,255,0.98),_rgba(248,250,252,0.92))] p-6 shadow-[0_30px_80px_-40px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-[radial-gradient(circle_at_top_left,_rgba(56,189,248,0.18),_transparent_38%),linear-gradient(135deg,_rgba(15,23,42,0.96),_rgba(2,6,23,0.96))]">
            <div className="flex flex-col items-center gap-5 text-center">
              ...
              <div>
                ...
                <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 dark:text-white">{displayName}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <ProfileSectionCard
            ...
            description="Read-only identity data synced from your profile record."
          >
            <div className="grid gap-4 md:grid-cols-2">
              {profileFacts.map((item) => (
                <ProfileReadonlyField key={item.label} label={item.label} value={item.value} hint={item.hint} />
              ))}
            </div>
          </ProfileSectionCard>
          ...
          <div className="flex flex-col-reverse gap-3 rounded-[28px] border border-slate-200/80 bg-white/90 p-4 shadow-[0_18px_50px_-36px_rgba(15,23,42,0.4)] backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:flex-row sm:items-center sm:justify-between">
